...
export const createSearchPayload = (search: z.infer<typeof SearchAPIResponseModel>): z.infer<typeof SearchModel> => ({
  topQuery: {
    results: search?.topquery?.data.map((item) => {
      return {
        ...
        album: decodeEntities(item?.more_info?.album),
        url: item?.perma_url,
        type: item?.type,
        language: item?.more_info?.language,
        description: decodeEntities(item?.description),
        primaryArtists: decodeEntities(item?.more_info?.primary_artists),
        singers: decodeEntities(item?.more_info?.singers)
      }
    }),
  ...
  songs: {
    results: search?.songs?.data.map((song) => {
      return {
        ...
        album: decodeEntities(song?.more_info.album),
        url: song?.perma_url,
        type: song?.type,
        description: decodeEntities(song?.description),
        primaryArtists: decodeEntities(song?.more_info?.primary_artists),
        singers: decodeEntities(song?.more_info?.singers),
  ...
  albums: {
    results: search?.albums?.data.map((album) => {
      return {
        ...
        artist: decodeEntities(album?.more_info.music),
        url: album?.perma_url,
        type: album?.type,
        description: decodeEntities(album?.description),
  ...
  artists: {
    results: search?.artists?.data.map((artist) => {
      return {
        id: artist?.id,
        title: decodeEntities(artist?.title),
        image: createImageLinks(artist?.image),
        type: artist?.type,
        description: decodeEntities(artist?.description),
  ...
  playlists: {
    results: search?.playlists?.data.map((playlist) => {
      return {
        ...
        description: decodeEntities(playlist?.description)
      }
    }),
...
export const createSearchAlbumPayload = (
  ...
  results: album.results.map((item) => ({
    id: item.id,
    name: decodeEntities(item.title),
    description: decodeEntities(item.header_desc),
    url: item.perma_url,
